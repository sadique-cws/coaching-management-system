...
const PrerequisitesForm = ({initialData, courseId}) => {
  ...
  return (
    <div className="mt-6 bg-slate-100 rounded-md dark:bg-gray-800 p-3">
        ...
        {!isEditing && (
                <p className={cn("text-sm mt-2 dark:text-gray-300", !initialData?.prerequisites && "italic text-slate-500")}>
                    {initialData?.prerequisites || "No prerequisites set for this course"}
                </p>
            )}
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3 mt-3">
                    <FormField
                        ...
                        render={({field}) => (
                            <FormItem>
                                <FormControl>
                                    <Input disabled={isSubmitting} className="bg-white" placeholder="e.g " {...field}/>
                                </FormControl>
                                <FormMessage/>
                            </FormItem>
                        )}/>
